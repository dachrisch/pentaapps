...
public class FloatingApiZenalEntryTest {
    ...
    public void testCreateZenalEntryList() throws Exception {
        ...
        final ZenalEntryList zenalEntryList = within(Project.MEDIASATURN).drivingTo("Anfahrt",
                from(datetime(2011, 5, 10, 7, 50)).till(9, 20)).working("Lasttest",
                from(datetime(2011, 5, 10, 9, 20)).till(17, 20)).drivingReturn("Rückfahrt",
                from(datetime(2011, 5, 10, 17, 50)).till(19, 20)).list();
        ...
        iterator.next();
        assertThat(iterator.next(), is(new ZenalEntry(Project.MEDIASATURN, datetime(2011, 5, 10, 9, 20), datetime(2011,
                5, 10, 17, 20), "Lasttest", Category.PROJECT)));
    ...
    public void testEntriesAreSortedByAscendingDate() throws Exception {
        final ZenalEntryList zenalEntryList = within(Project.MEDIASATURN).drivingReturn("Rückfahrt",
                from(datetime(2011, 5, 10, 17, 50)).till(19, 20)).working("Lasttest",
                from(datetime(2011, 5, 10, 9, 20)).till(17, 20)).drivingTo("Anfahrt",
                from(datetime(2011, 5, 10, 7, 50)).till(9, 20)).list();
