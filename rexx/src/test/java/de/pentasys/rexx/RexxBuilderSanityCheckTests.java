...
public class RexxBuilderSanityCheckTests {
    ...
    public void tripsCreatedIsInsideJournedDate() {
        doJourney(Project.MEDIASATURN).starting(datetime(2011, 5, 5, 7, 50)).from("München")
    ...
    public void tripCreatedIsBeforeJournedDate() {
        try {
            doJourney(Project.MEDIASATURN).starting(datetime(2011, 5, 5, 7, 50)).from("München")
                    .till(datetime(2011, 5, 15, 17, 50)).to("Ingolstadt")
                    .withTrip(from(datetime(2011, 5, 5, 7, 40)).till(datetime(2011, 5, 15, 17, 50)), "projekteinsatz");
    ...
    public void tripCreatedIsAfterJournedDate() {
        try {
            doJourney(Project.MEDIASATURN).starting(datetime(2011, 5, 5, 7, 50)).from("München")
                    .till(datetime(2011, 5, 15, 17, 50)).to("Ingolstadt")
                    .withTrip(from(datetime(2011, 5, 5, 7, 50)).till(datetime(2011, 5, 15, 17, 51)), "projekteinsatz");
